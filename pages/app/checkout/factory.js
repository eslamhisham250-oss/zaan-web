...
import { getCartFactory, totalCartFactory, clearCartFactory } from "../../../lib/cartFactory";

export default function FactoryCheckout() {
  const [form, setForm] = useState({ name: "", phone: "", email: "", address: "", note: "" });
  const [paymentMethod, setPaymentMethod] = useState("card"); // افتراضي بطاقة
  const [loading, setLoading] = useState(false);
  const [success, setSuccess] = useState(null);

  const total = totalCartFactory();
        ...
        {success ? (
          ...
          <>
            <div style={{ marginBottom: 20, padding: 16, background: "#f9f9f9", borderRadius: 8 }}>
              <h3>ملخص الطلب</h3>
              <p>عدد المنتجات: {getCartFactory().reduce((s, i) => s + i.qty, 0)}</p>
              <p>الإجمالي: {total} ج.م</p>
            </div>
